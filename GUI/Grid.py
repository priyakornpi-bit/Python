...
class CalculatorLayout(QWidget):
    ...
    def __init__(self):
        ...
        # ==================== Formula Bar (บาร์สำนวน) ด้านบน ====================
        self.formula_bar = QLabel("")  # แสดงสำนวนการคำนวณ (เช่น "5 +")
        self.formula_bar.setAlignment(Qt.AlignRight | Qt.AlignVCenter)  # ชิดขวา
        self.formula_bar.setMinimumHeight(30)  # ความสูง
        self.formula_bar.setStyleSheet("font-size: 14px; color: gray;")  # ฟอนต์เล็ก สีเทา
        layout.addWidget(self.formula_bar, 0, 0, 1, 4)  # วาง row 0
        ...
        layout.addWidget(self.display, 1, 0, 1, 4)  # วาง row 1
    ...
    def on_clear(self):
        ...
        self.new_number = True
        self.update_display()
        self.formula_bar.setText("")  # ล้าง formula bar ด้วย
    ...
    def update_display(self):
        ...
        self.update_formula_bar()  # อัพเดต formula bar ด้วย

    # ==================== ฟังก์ชันอัพเดต Formula Bar ====================
    def update_formula_bar(self):
        """
        อัพเดตบาร์สำนวน (formula bar) ที่ด้านบน
        แสดงสำนวนการคำนวณปัจจุบัน เช่น "5 +"
        """
        if self.operator is None:
            # ไม่มีตัวดำเนินการ -> formula bar ว่าง
            self.formula_bar.setText("")
        else:
            # มีตัวดำเนินการ -> แสดง "5 +"
            if self.previous_value is not None:
                self.formula_bar.setText(f"{int(self.previous_value) if self.previous_value == int(self.previous_value) else self.previous_value} {self.operator}")
